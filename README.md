...
Drag a camera from the left panel onto the black canvas. Drag the camera to move it, or drag any blue corner handle to resize it. A camera can extend beyond the canvas edge when only part of its view should be recorded. Right-click a camera tile and select **Rotate 180°** for an upside-down mounted camera. Use **Refresh** above the camera list after connecting a camera while the app is open. Double-clicking a camera in the list also adds it. Select a camera and press Delete to remove it. Each camera can appear once. The visible camera area is center-cropped to its tile, with no stretching; uncovered canvas remains black.
...
The recorder always creates 1080p (1920×1080), 30 fps H.264 MP4 files using the former **High** quality setting. Press **Record** and **Stop** to finalize the MP4. Files are saved automatically in your Windows **Videos** folder. The output contains one H.264 video stream and no audio. There is no playback screen.
